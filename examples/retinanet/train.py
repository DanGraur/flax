...
def coco_eval_step(bboxes: np.ndarray, scores: np.ndarray, labels: np.ndarray,
                   usable_rows: np.ndarray, img_ids: np.ndarray,
                   scales: np.ndarray, evaluator: CocoEvaluator):
  """Adds a set of batch inferences to the COCO evaluation object

  Args:
    bboxes: an array of the shape (N, A, 4), where `N` is the batch size, and
      `A` is the number of anchors
    scores: an array of the shape (N, A), which stores the anchor confidence 
  ...
  evaluator.add_annotations(bboxes, scores, labels, usable_rows, img_ids,
                            scales)
...
def post_process_inferences(inference_dict: Mapping[str, jnp.ndarray],
...
def eval_to_tensorboard(writer: tensorboard.SummaryWriter,
                        evals: Iterable,
                        step: int,
                        train: bool = True,
                        aggregate: bool = True):
  ...
  for step in range(start_step, config.num_train_steps + config.warmup_steps):
    ...
    if step % config.checkpoint_period == 0 and step != 0:
      ...
      for eval_step, batch in enumerate(val_data):
        ...
        bboxes, scores, labels, usable_rows = process_inferences_partial_fn(
            inference_dict)
        coco_eval_step(bboxes, scores, labels, usable_rows,
                       batch["id"].reshape(-1), batch["scale"].reshape(-1),
                       coco_evaluator)
